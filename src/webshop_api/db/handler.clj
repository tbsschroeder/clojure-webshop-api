...
(defn create-table! [connection is-testing]
  ...
  (let [create-table "CREATE TABLE IF NOT EXISTS article"
        columns "title TEXT NOT NULL, description TEXT NOT NULL, category TEXT NOT NULL, image TEXT NOT NULL, count INTEGER DEFAULT 0 NOT NULL"
        test-id "id INTEGER PRIMARY KEY"
        prod-id "id SERIAL"
        test-db (str create-table "(" test-id ", " columns ")")
        prod-db (str create-table "(" prod-id ", " columns ")")]
    (if is-testing
      (jdbc/execute! connection test-db)
      (jdbc/execute! connection prod-db)))
